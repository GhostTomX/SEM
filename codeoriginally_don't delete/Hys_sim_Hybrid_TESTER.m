...
switch Prsh_Mode
    ...
    case {2}
        ab0=0;
        Bias_chsn= 0.6238-0.9405;%0.6108-0.8737;%-0.555;  % Dynamic is proper
end
...
figure;
hold on
plot(t(j : k),Plot_element*1000) ;
legend(Plot_legend) ;
title([Simulation_Title,' Lv',num2str(Speed_level)]) ;
savefig('Uh Real & Uh Simulate');

plot(t(j : k),Error_element*1000) ;
hold off
...
title([Error_Title,' Lv',num2str(Speed_level),' order',num2str(order)]);
% savefig('Uh Simulation Error');

figure;
plot(ref(j : k),Plot_element,ref(j : k),ref(j : k)); title(['Ref - Uh',' Lv',num2str(Speed_level)]); legend([ Plot_legend ; '1 : 1 Line  '],2) ; xlabel('ref'); ylabel('Uh');
title([Error_Title,' Lv',num2str(Speed_level),' order',num2str(order)]);
savefig('Uh Real & Preisach Simulate');
% 
% figure;
% plot(ref(j : k),Plot_element(:,1),ref(j : k),ref(j : k)); title(['Ref - Uh',' Lv',num2str(Speed_level)]); legend([ Plot_legend(:,1) ; '1 : 1 Line  '],2) ; xlabel('ref'); ylabel('Uh');
