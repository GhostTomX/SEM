...
double			ustatei[TotalPts];
double			ustateo[TotalPts];
...
// (1) PID
double		Kp = 0.4;		 // SF 0.15;   Ethan 0.45
double		Ki = 0.5;		 //  SF 0.2;    Ethan 0.15
// (2) F(z)
...
double		y_Fili[TotalPts];
double		u_ctrli[TotalPts];   // linear control part of control input
double      u_ctrl_previ;
double		errsumi = 0;
double		ei;

double		y_Filo[TotalPts];
double		u_ctrlo[TotalPts];   // linear control part of control input
double      u_ctrl_prevo;
double		errsumo = 0;
double		eo;
...
double		Upo[TotalPts];
// ======= Inner Loop Preisach =================
int		    Prsh_use = 1;		// 是否使用 Preisach (0 NULL / 1 變動量 / 2 完整Preisach )
int		    Prsh_include = 1;
int			Pvar_flt_use = 1;
...
double		Prshi[TotalPts], Prsh_var_Flti[TotalPts], Prsh_Flti[TotalPts];
double		Prsho[TotalPts], Prsh_var_Flto[TotalPts], Prsh_Flto[TotalPts];
double		Prsh_cmp;
double		Prsh_cmpi;
double		Prsh_cmpo;
...
void        strlink(char *lhs, char *rhs);
...
int main(void)
{
	...
	fp = fopen("Multi_Step_H2.8_t15_Clmb7_F_ord8_30Hz.txt", "r");
...
/*	if (order == 6){
	...
	for(timesOfExp=0;timesOfExp<timesOfExpG;i++){
	printf("delaystart");
	Sleep(3000);
	printf("delayover");
	timesOfExp++;
	...
	for (j = pre_points+0; j < pre_points+data_points; j++)
	{
		...
		N1231AGetRawPosVel(hN1231A, 0, &p2, &v2);
		yo[j] = -p2 * 0.30908 / 1000;
		volo[j] =-v2;
		...
		*/
		...
		Upi[j] = (84 * ve[j] - Tem * yi[j - 1]) / 72;	
		Upo[j] = (84 * ve[j] - Tem * yo[j - 1]) / 72;
		//================================================= Outer Loop Control
		if (Outer_use == 1){
			// (1) y_Fil = F(z) * y 
			y_Fili[j] = Fil*y_Fili[j - 1] + (1 - Fil)*yi[j];
			y_Filo[j] = Fil*y_Filo[j - 1] + (1 - Fil)*yo[j];
			...
			// e = r-y
			// (3) u_ctrl = C(z) * e
			errsumi += ei;										// Integrate error
			errsumo += eo;										// Integrate error

			u_ctrli[j] = Kp*ei + Ki*errsumi;						// PID control
			u_ctrlo[j] = Kp*eo + Ki*errsumo;						// PID control
		}
		else{
			u_ctrli[j] = r[j];
			u_ctrlo[j] = r[j];
		...
		if (Middle_use == 1){
			u_ctrl[j] = Amp * u_ctrli[j] + cf * ve[j] / 10;
		...
		{
			//ustate[j] += (-K_s[i] * x[i]);
			ustatei[j] += (-K_s[i] * xi[i]);
			ustateo[j] += (-K_s[i] * xo[i]);
		}
		for (i = 0; i<4; i++)
		{
			//xdot[i] = B_s[i] * (u_ctrl[j] + ustate[j]) + ax[i] + L_s[i] * (y[j] - yhat[j]) + oion*0.001*L_s[i] * eysum;
			xdoti[i] = B_s[i] * (u_ctrli[j] + ustate[j]) + ax[i] + L_s[i] * (yi[j] - yhat[j]) + oion*0.001*L_s[i] * eysumi;
			xdoto[i] = B_s[i] * (u_ctrlo[j] + ustate[j]) + ax[i] + L_s[i] * (yo[j] - yhat[j]) + oion*0.001*L_s[i] * eysumo;
		}
		for (i = 0; i < 4; i++)
		{
			//x[i] = xdot[i];
		...
		ui[j] = u_ctrli[j] + Vh_cmp[j] + A_cmp[j] + Prsh_cmpi + sf*ustatei[j];
		uo[j] = u_ctrlo[j] + Vh_cmp[j] + A_cmp[j] + Prsh_cmpo + sf*ustateo[j];
		//=================================================Inner Loop (2) - Preisach Model
		if (Prsh_use == 1){                 // Preisach Var 補償
			// (1) Preisach Model
			//Prsh[j] = Preisach_WPout(u, j);
			Prshi[j] = Preisach_WPout(ui, j);
			Prsho[j] = Preisach_WPout(uo, j);

			//Prsh_var = Prsh[j] - Prsh[j - 1];
			Prsh_vari = Prshi[j] - Prshi[j - 1];
			Prsh_varo = Prsho[j] - Prsho[j - 1];
			...
			if (Pvar_flt_use == 1){
				...
				Prsh_var_Flti[j] = (f_Prsh_alpha)*Prsh_var_Flti[j - 1] + (1 - f_Prsh_alpha)*Prsh_vari;
				Prsh_cmpi = Prsh_var_Flti[j];
				Prsh_var_Flto[j] = (f_Prsh_alpha)*Prsh_var_Flto[j - 1] + (1 - f_Prsh_alpha)*Prsh_varo;
				Prsh_cmpo = Prsh_var_Flto[j];
			}
			else{
				//Prsh_cmp = Prsh_var;
				Prsh_cmpi = Prsh_vari;
			...
			Prsh_cmpi = Prsh_var_Amp * Prsh_cmpi;
			Prsh_cmpo = Prsh_var_Amp * Prsh_cmpo;
		}
		...
		if (Prsh_include == 1){
			...
			piezo[0] = uo[j] + Prsh_cmpo;
			piezo[1] = ui[j] + Prsh_cmpi;
		}		
		if (HysCmp == 0){
			piezo[0] = u_ctrl[j] ;
			piezo[1] = u_ctrl[j] ;
	...
	//gcvt(Ki,3,Kichar);
	//	===== Save Data =====
	char *name_file = "Measurement_Data_Kp_";
	//strlink(name_file, Kpchar);
	//strcat(name_file, "_Ki_");
	//strcat(name_file, Kichar);
	char *name_end = ".txt";
	strlink(name_file, name_end);
	printf("%s", name_file);
	...
	for (i= pre_points+0 ; i<pre_points+data_points ; i++)
	{
		fprintf(fp,"%f %f %f %f %f %f %f %f %f\n", yi[i], ui[i], ve[i], u_ctrl[i], vol[i],yo[i], volo[i],p[i],vin[i]);
	}
	fclose(fp);	
