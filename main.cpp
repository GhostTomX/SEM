...
double		Ki = 0.235;		 //  SF 0.2;    Ethan 0.15
// (2) F(z)
...
/*	if (order == 6){
	...
	for (j = pre_points+0; j < pre_points+data_points; j++)
	{
		...
		*/
		...
		ui[j] = u_ctrli[j] + Vh_cmp[j] + A_cmp[j] + Prsh_cmpi + sf*ustatei[j] + Kd*diffsum;
		uo[j] = u_ctrlo[j] + Vh_cmp[j] + A_cmp[j] + Prsh_cmpo + sf*ustateo[j] - Kd*diffsum;
